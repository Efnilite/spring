package dev.efnilite.spring.config;
...
public enum Config {
    ...
    public static void reload() {
        ...
        Spring.logging().info("Loaded all config files.");
    }
    ...
    Config(String fileName, @Nullable List<String> ignoredSections) {
        ...
        this.path = Spring.getInFolder(fileName);

        if (!path.exists()) {
            Spring.getPlugin().saveResource(fileName, false);
            Spring.logging().info("Created config file %s".formatted(fileName));
        }
    ...
    public void update() {
        try {
            ConfigUpdater.update(Spring.getPlugin(), fileName, path, ignoredSections);
        } catch (Exception ex) {
            Spring.logging().stack("Error while trying to update config file", ex);
        }
    }
